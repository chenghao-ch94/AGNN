...
def compute_acc2(logits, label, reduction='mean'):
    lab_node = torch.argmax(logits, dim=1)/5
    ret = (lab_node == label).float()
    if reduction == 'none':
        return ret.detach()
    elif reduction == 'mean':
        return ret.mean().item()
...
def make_optimizer(params, name, lr, weight_decay=None, milestones=None):
    ...
    if milestones:
        ...
        # lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=100,eta_min=1.e-6)
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1500, gamma=0.1)
